...
public class QueryPlan {
    ...
    public QueryOperator minCostSingleAccess(String table) {
        ...
        int finalIndexCol = -1;
        //System.out.println(eligibleIndexColumns);
        for (int indexCol : eligibleIndexColumns) {
            String columnName = selectPredicates.get(indexCol).column;
            PredicateOperator curPre = selectPredicates.get(indexCol).operator;
            DataBox curDataBox = selectPredicates.get(indexCol).value;
            IndexScanOperator indexScanOperator = new IndexScanOperator(transaction, table, columnName, curPre, curDataBox);
            if (cost > indexScanOperator.estimateIOCost()) {
...
//                    rightOp = prevMap.get(set);
                ...
                //System.out.println("!!" + leftOp.toString() +  rightOp.toString());

                subset.addAll(set);
                QueryOperator subResOp = minCostJoinType(leftOp, rightOp, leftColumn, rightColumn);

                if (!result.containsKey(subset)) {
                    result.put(subset, subResOp);
                } else if (subResOp.estimateIOCost() < result.get(subset).estimateIOCost()) {
                    result.put(subset, subResOp);
                }
            }
        }
    ...
    public Iterator<Record> execute() {
        ...
        for (String table : tableNames) {
            singleTableQueryPlan.add(minCostSingleAccess(table));
        }
        //
        ...
        for (int i = 0; i < tableNames.size(); i++) {
            ...
            set.add(tableName);
            pass1Map.put(set, singleTableQueryPlan.get(i));
        }
